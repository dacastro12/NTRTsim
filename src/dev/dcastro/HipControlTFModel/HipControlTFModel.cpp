...
HipControlTFModel::HipControlTFModel() :
tgModel()
{
}
...
void HipControlTFModel::addNodes(tgStructure& s,
                            double height,
                            double femur,
                            double fibia)
{
...
//femur
    ...
    // top front left
    s.addNode(0.25*height*femur, height*fibia + height*femur, 0.25*height*femur);// 14
    //top back left
    s.addNode(0.25*height*femur, height*fibia + height*femur, -0.25*height*femur); //15
    // top back 
    s.addNode(0, height*fibia + height*femur, -0.25*height*femur); // 16
    // top back right
    s.addNode( -0.25*height*femur, height*fibia + height*femur, -0.25*height*femur); // 17
    // top front right
    s.addNode(-0.25*height*femur, height*fibia + height*femur, 0.25*height*femur); // 18
    // top right mid
    s.addNode(-0.25*height*femur, height*fibia + height*femur, 0);//19
    // top left mid
    s.addNode(0.25*height*femur, height*fibia + height*femur, 0);//20
...
//Pyramid Design
    // top front left
    s.addNode(0.35*height*femur, height*fibia + height*femur, 0.125*height*femur);// 24
    //top back left
    s.addNode(0.35*height*femur, height*fibia + height*femur, -0.125*height*femur); //25
    // top back right
    s.addNode( -0.35*height*femur, height*fibia + height*femur, -0.125*height*femur); //26
    // top front right
    s.addNode(-0.35*height*femur, height*fibia + height*femur, 0.125*height*femur); // 27
    // top  front origin
    s.addNode(0, height*fibia + height*femur + 0.25*height*femur, 0.125*height*femur);//28
    // top back origin
    s.addNode(0, height*fibia + height*femur + 0.25*height*femur, -0.125*height*femur);//29
     // front point
    s.addNode(0, height*fibia + height*femur + 0.25*height*femur, 0.4*height*femur);//30
    // back point
    s.addNode(0, height*fibia + height*femur + 0.25*height*femur, -0.4*height*femur);//31
    

}

/*void HipControlTFModel::addNodesB(tgStructure& s,
                            double edge,
                            double width,
                            double height)
{
...
*/
void HipControlTFModel::addRods(tgStructure& s)
{
//fibula and tibia
...
//Hip
	//Pyramid Model
	s.addPair(24, 28, "rodB");
	s.addPair(27, 28, "rodB");
	s.addPair(25, 29, "rodB");
	s.addPair(26, 29, "rodB");
        //s.addPair(28, 29, "rodB");
	//s.addPair(28, 30, "rodB");
	s.addPair(30, 31, "rodB");
	
}

/*void HipControlTFModel::addPairsB(tgStructure& s)
{
...
*/
void HipControlTFModel::addMuscles(tgStructure& s)
{
...
//Hip Section
	//Joint
	s.addPair(14, 24, "muscle"); //Left side
	s.addPair(20, 24, "muscle");
	s.addPair(20, 25, "muscle");
	s.addPair(15, 25, "muscle");
	
	s.addPair(17, 26, "muscle");// Right side
	s.addPair(19, 26, "muscle");
	s.addPair(19, 27, "muscle");
	s.addPair(18, 27, "muscle");

	s.addPair(28, 12, "muscle");//Top
	s.addPair(29, 12, "muscle");

	//Movement Muscles (Not yet Implemented)
	s.addPair(11, 24, "muscle");
	s.addPair(9, 25, "muscle");
	s.addPair(10, 26, "muscle");
	s.addPair(11, 27, "muscle");
	s.addPair(30, 8, "muscle");
...
void HipControlTFModel::setup(tgWorld& world)
{
...
void HipControlTFModel::step(double dt)
{
    // Precondition
    if (dt <= 0.0)
    {
...
void HipControlTFModel::onVisit(tgModelVisitor& r)
{
    // Example: m_rod->getRigidBody()->dosomething()...
    tgModel::onVisit(r);
}

const std::vector<tgBasicActuator*>& HipControlTFModel::getAllMuscles() const
{
    return allMuscles;
}

void HipControlTFModel::teardown()
{
    notifyTeardown();
    tgModel::teardown();
}
